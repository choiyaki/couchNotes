...
final class SyncEngine {
    ...
    func flush() async {
        guard NetworkMonitor.shared.isOnline, !flushing else { return }
        flushing = true
        ...
        for id in await NoteStore.shared.pendingDeleteIDs() {
            do {
                try await CouchDBClient.shared.deleteNote(id: id)
                await NoteStore.shared.removeRow(id)
                changed = true
        ...
        for id in await NoteStore.shared.dirtyIDs() where id != activeNoteId {
            guard let s = await NoteStore.shared.editingNote(id) else { continue }
            ...
            do {
                ...
                await NoteStore.shared.markClean(id, rev: newRev)
                changed = true
            } catch CouchDBError.networkError {
                return
                ...
                if (try? await CouchDBClient.shared.fetchNoteRecord(id: id)) == nil,
                   let tomb = try? await CouchDBClient.shared.currentLeafRev(id: id),
                   let newRev = try? await CouchDBClient.shared.saveNoteContentChecked(
