(() => {
  ...
  var TileBuilder = class {
    ...
    addComposition(composition, context) {
      ...
      for (let i = context.marks.length - 1; i >= 0; i--) {
        let mark2 = context.marks[i];
        let last = head.lastChild;
        if (last instanceof MarkTile && last.mark.eq(mark2.mark)) {
          if (last.dom != mark2.dom)
            last.setDOM(freeNode(mark2.dom));
          head = last;
        } else {
    ...
    ensureMarks(marks2, openStart) {
      ...
      for (let i = marks2.length - 1; i >= 0; i--) {
        let mark2 = marks2[i], last;
        if (openStart > 0 && (last = parent.lastChild) && last instanceof MarkTile && last.mark.eq(mark2)) {
          parent = last;
          openStart--;
        } else {
  ...
  // webview/state.ts
  var setWikiTargets = StateEffect.define();
  var wikiTargetsField = StateField.define({
    create: () => [],
    update(value, tr) {
      for (const e of tr.effects) {
        if (e.is(setWikiTargets))
          return e.value;
      }
      return value;
    }
  ...
  // webview/decorations.ts
  var TAB_SIZE = 2;
  var WIKI_RE = /\[\[([^\]]+)\]\]/g;
  var MD_LINK_RE = /(?<!!)\[[^\]]*\]\((https?:\/\/[^)\s]+)\)/g;
  var IMG_MD_RE = /!\[[^\]]*\]\((https?:\/\/[^)\s]+)\)/g;
  var BARE_URL_RE = /https?:\/\/[^\s)\]]+/g;
  var BLOCK_ID_RE = /\^[a-zA-Z0-9_-]+$/;
  ...
  function styleLine(view2, lineFrom, lineNumber, text, out, ctx) {
  ...
  function styleInline(text, lineFrom, active, out, ctx) {
    const covered = [];
    IMG_MD_RE.lastIndex = 0;
    for (let m; m = IMG_MD_RE.exec(text); ) {
    ...
    for (let m; m = MD_LINK_RE.exec(text); ) {
      const s = lineFrom + m.index;
      const e = s + m[0].length;
    ...
    for (let m; m = BARE_URL_RE.exec(text); ) {
  ...
  function build(view2) {
    const names = view2.state.field(wikiTargetsField);
    const activeLines = /* @__PURE__ */ new Set();
    for (const r of view2.state.selection.ranges) {
      const a = view2.state.doc.lineAt(r.from).number;
      const b = view2.state.doc.lineAt(r.to).number;
      for (let n = a; n <= b; n++)
        activeLines.add(n);
    }
    const ctx = { wiki: new Set(names.map((n) => n.toLowerCase())), activeLines };
    const out = [];
  ...
  var liveStyling = ViewPlugin.fromClass(
    class {
      decorations;
      constructor(view2) {
        this.decorations = build(view2);
      }
      update(u) {
        if (u.docChanged || u.viewportChanged || u.selectionSet || // カーソル移動で画像 Markdown の淡色 ON/OFF を切り替える
  ...
  function parseLine(text) {
    const specs = [];
    IMG_RE.lastIndex = 0;
    for (let m; m = IMG_RE.exec(text); ) {
      const w = /\|\s*(\d+)/.exec(m[1]);
      specs.push({ url: m[2], widthPct: w ? Number(w[1]) : null });
    }
    return specs;
  }
  var naturalSizes = /* @__PURE__ */ new Map();
  var editorWidth = 0;
  var viewRef = null;
  var imagesChanged = StateEffect.define();
  function contentPadding(view2) {
  ...
  function availableWidth(view2) {
    const pad = contentPadding(view2);
    return view2.contentDOM.clientWidth - pad.left - pad.right;
  }
  ...
  function groupSizes(specs, width) {
    ...
    return specs.map((s) => {
      ...
      if (!nat)
        return { w: width * 0.5, h: PLACEHOLDER_HEIGHT };
      const half = width * 0.5;
      return nat.w >= nat.h ? { w: half, h: half * nat.h / nat.w } : { w: half * nat.w / nat.h, h: half };
    });
  }
  function buildPaddings(state) {
  ...
  var imagePaddingField = StateField.define({
  ...
  var imageOverlay = ViewPlugin.fromClass(
    class {
      constructor(view2) {
        this.view = view2;
        viewRef = view2;
        view2.scrollDOM.style.position = "relative";
        this.container = document.createElement("div");
        ...
        view2.scrollDOM.appendChild(this.container);
        this.ro = new ResizeObserver(() => this.syncWidth());
        this.ro.observe(view2.scrollDOM);
        this.schedule();
      }
      container;
      ro;
      ...
      update(u) {
        if (u.docChanged || u.viewportChanged || u.geometryChanged || u.transactions.some((t2) => t2.effects.some((e) => e.is(imagesChanged)))) {
          this.schedule();
        }
      }
      schedule() {
        this.view.requestMeasure({
          read: () => this.read(),
          write: (placed) => this.write(placed)
        });
      }
      read() {
        const view2 = this.view;
        const width = editorWidth || availableWidth(view2) || 360;
        const scrollerRect = view2.scrollDOM.getBoundingClientRect();
        const contentRect = view2.contentDOM.getBoundingClientRect();
        const scrollTop = view2.scrollDOM.scrollTop;
        const originX = contentRect.left - scrollerRect.left + contentPadding(view2).left;
        const docTop = view2.documentTop;
        const placed = [];
        ...
        for (let i = 1; i <= doc2.lines; i++) {
          ...
          if (specs.length === 0)
            continue;
          const block = view2.lineBlockAt(line.from);
  ...
  var imageField = [imagePaddingField, imageOverlay];
  ...
  function placeholderText(id) {
    return `![\u30A2\u30C3\u30D7\u30ED\u30FC\u30C9\u4E2D\u2026](upload://${id})`;
  }
  function insertUploadPlaceholder(view2) {
    const id = genId();
    ...
    return EditorView.domEventHandlers({
      paste(event, view2) {
        ...
        if (!file)
          return false;
        event.preventDefault();
        const id = insertUploadPlaceholder(view2);
        ...
        reader.onload = () => {
          ...
          native2.postMessage({ type: "pasteImage", id, mime, data, filename: `image.${ext}` });
        };
        reader.onerror = () => applyPasteResult(view2, { id, error: "read-failed" });
        reader.readAsDataURL(file);
        return true;
      }
    });
  }
  function applyPasteResult(view2, msg) {
    const placeholder = placeholderText(msg.id);
    const idx = view2.state.doc.toString().indexOf(placeholder);
    if (idx < 0)
      return;
  ...
  function setFooterPoster(p) {
    nativeRef = p;
  }
  function setFooterData(data) {
    currentData = data;
    render();
    viewRef2?.requestMeasure();
  }
  ...
  function sectionHeader(text, targetId) {
  ...
  function render() {
    ...
    const hasContent2 = !!data && (data.backlinks.length > 0 || data.twoHop.length > 0);
    container.style.display = hasContent2 ? "block" : "none";
    if (!data || !hasContent2)
      return;
    ...
    for (const [mode, glyph] of [["list", "\u2630"], ["grid", "\u25A6"]]) {
    ...
    if (data.backlinks.length > 0) {
      ...
      for (const n of data.backlinks)
        wrap.appendChild(noteButton(n));
  ...
  var footerPlugin = ViewPlugin.fromClass(
    class {
      constructor(view2) {
        this.view = view2;
        viewRef2 = view2;
        ...
        view2.scrollDOM.appendChild(el);
        containerRef = el;
        render();
        this.schedule();
      }
        ...
        this.view.requestMeasure({
          read: () => {
            ...
            return {
              ...
              top: contentRect.bottom - padBottom - scrollerRect.top + view2.scrollDOM.scrollTop + 24,
              left: contentRect.left - scrollerRect.left + padLeft,
              width: view2.contentDOM.clientWidth - padLeft - padRight
            };
          },
          write: (m) => {
      ...
      destroy() {
        containerRef?.remove();
        containerRef = null;
        if (viewRef2 === this.view)
          viewRef2 = null;
      }
    }
  );
  var footerExtension = footerPlugin;
  ...
  function nextListMarker(stripped) {
    if (/^[-*] \[ \]/.test(stripped))
      return { marker: stripped[0] + " [x] ", oldLen: 6 };
    if (/^[-*] \[x\]/i.test(stripped))
      return { marker: stripped[0] + " ", oldLen: 6 };
    if (/^[-*] /.test(stripped))
      return { marker: stripped[0] + " [ ] ", oldLen: 2 };
    if (/^\+ /.test(stripped))
      return { marker: "- [ ] ", oldLen: 2 };
    return { marker: "- ", oldLen: 0 };
  }
  function listContinuation(view2) {
    const { state } = view2;
    ...
    if (afterMarker.trim() === "") {
      ...
      const insert2 = "\n" + tabs + nextMarker;
      view2.dispatch({
        changes: { from: sel.head, to: sel.head, insert: insert2 },
        selection: { anchor: sel.head + insert2.length },
        userEvent: "input"
      });
    }
    return true;
  }
  function indentLines(view2) {
    const lines = selectedLines(view2.state);
    view2.dispatch({
  ...
  function outdentLines(view2) {
    const lines = selectedLines(view2.state);
    const changes = lines.filter((l) => l.text.startsWith("	")).map((l) => ({ from: l.from, to: l.from + 1 }));
    if (changes.length === 0)
      return false;
    view2.dispatch({ changes, userEvent: "delete.dedent" });
    return true;
  }
  function toggleListMarker(view2) {
    const { state } = view2;
    ...
    const changes = lines.map((l) => {
      ...
      if (stripped.trim() === "")
        return { from: l.from, to: l.to, insert: l.text };
      const oldLen = nextListMarker(stripped).oldLen;
      const cur2 = curMarkerLen(stripped);
      return { from: l.from, to: l.to, insert: tabs + target + stripped.slice(cur2) };
    });
    view2.dispatch({ changes, userEvent: "input" });
    return true;
  }
  function curMarkerLen(stripped) {
    if (/^[-*] \[[ xX]\]/.test(stripped))
      return 6;
    if (/^[-*+] /.test(stripped))
      return 2;
  ...
  function insertWikiLink(view2) {
    const sel = view2.state.selection.main;
    const selected = view2.state.sliceDoc(sel.from, sel.to);
  ...
  function toggleCheckboxAt(view2, pos) {
    const line = view2.state.doc.lineAt(pos);
    const m = /^([\t ]*)[-*] \[([ xX])\]/.exec(line.text);
    if (!m)
      return false;
    const boxCharPos = line.from + m[1].length + 3;
    const next = m[2] === " " ? "x" : " ";
    view2.dispatch({
  ...
  var clickHandler = EditorView.domEventHandlers({
    mousedown(e, view2) {
      ...
      if (hit) {
        if (hit.kind === "wiki")
          ...
          native.postMessage({ type: "openExternal", url: hit.value });
        e.preventDefault();
        return true;
      }
      return false;
  ...
  window.couchNotesRunCommand = (name2) => {
    switch (name2) {
      case "wikiLink":
        insertWikiLink(view);
        break;
      case "listToggle":
        toggleListMarker(view);
        break;
      case "moveLineUp":
        moveLineUp(view);
        break;
  ...
  window.couchNotesReceive = (msg) => {
    switch (msg?.type) {
      ...
      case "insertText": {
        ...
        view.dispatch({
          changes: { from: sel.from, to: sel.to, insert: t2 },
          selection: { anchor: sel.from + t2.length },
          userEvent: "input.paste"
        });
        view.focus();
        break;
